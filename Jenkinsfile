pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'web-back-auctions-k8s'
        GITHUB_REPO = 'https://github.com/jbnu-web-class-project/auctions.git'
                ...
                script {
                    sshagent(credentials: [SSH_CREDENTIALS_ID]) {
                        ...
                        script {
                            ...
                            cd k8s-manifest &&
                            ls -l &&
                            sed -i 's|image: .*\$|image: ${HARBOR_REPO}/heim/${IMAGE_NAME}:${env.BUILD_ID}|' auctions/back-auctions-deploy.yaml &&
                            git config --global user.email "gjdhks1212@gmail.com"
                            git config --global user.name "hodu26"
                            git add auctions/back-auctions-deploy.yaml &&
